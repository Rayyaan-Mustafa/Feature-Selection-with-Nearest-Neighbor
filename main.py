...
def automate_tests():
    results = []
    files = ["CS170_Large_Data__21.txt","CS170_Large_Data__99.txt","CS170_Small_Data__88.txt","CS170_Large_Data__6.txt","CS170_Small_Data__6.txt","CS170_Small_Data__96.txt","CS170_Large_Data__96.txt","CS170_Small_Data__64.txt"]
    start = time.time()
    end = 0
    for f in files:
        try:
            data= np.loadtxt(f)
            #runs forward selection on each file
            ...
            results.append((f,OVERALL_best_accuracy,OVERALL_best_set,end-start))
...
def main():
    ...
    print("Running nearest neighbor with all " + str(len(data[0]) - 1) + " features, using \"leaving-one-out\" evaluation, I get an accuracy of " + str(leave_one_out_cross_validation(data, set(range(1,len(data[0]))), 0)*100) + "%")
...
def leave_one_out_cross_validation(data, current_set_of_features, feature_to_add):
    ...
    # print("feature_to_add" + str(feature_to_add))
